...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (error) return jsonError(error.message || 'Admin notes fetch failed', 500);

    return NextResponse.json({ ok: true, notes: (data ?? []).map((row) => mapNote(row as AdminNoteRow)) });
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Admin notes fetch failed';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error) return jsonError(error.message || 'Admin note insert failed', 500);

    return NextResponse.json({ ok: true, noteId: data.id });
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Admin note insert failed';
    return jsonError(message, 500);
  }
}
