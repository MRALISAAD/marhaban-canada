...
export async function GET() {
  try {
    ...
    if (error) {
      return jsonError(error.message || 'Resources fetch failed', 500);
    }
...
export async function POST(request: Request) {
  ...
  if (!isRecord(body)) {
    return jsonError('Invalid resource payload', 400);
  }

  if (!hasOnlyAllowedFields(body)) {
    return jsonError('Unexpected resource field rejected', 400);
  }
  ...
  try {
    ...
    if (error) {
      return jsonError(error.message || 'Resource insert failed', conflictStatus(error));
    }

    return NextResponse.json({ ok: true, resourceId: data.id });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Resource insert failed';
    return jsonError(message, 500);
  }
}
